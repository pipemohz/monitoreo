...
files = [file for file in listdir(
    folder) if 'report' in file and file.endswith(".png")]
...
for file, message in zip(files, messages):
    ...
    threshold_image = cv2.threshold(
        gray_redux, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
    ...
    details = pytesseract.image_to_data(
        threshold_image, output_type=Output.DICT, config=custom_config, lang='spa')
